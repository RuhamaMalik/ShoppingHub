    ...
    try {
        ...
        if (searchKey === 'createdAt' || searchKey === 'updatedAt') {
            ...
            categories = await Category.findAll({
                where: {
                    [searchKey]: {
                        [Op.between]: [searchDate, new Date(searchDate.getTime() + 86400000)] // track fulll day
                    }
                }
            });
        } else {
        ...
        res.status(200).json({ success: true, categories: formattedCategories, message: 'Filtered Successfully' });
    } catch (error) {
    ...
    try {
        ...
        if (req.file) {
            ...
            fs.readdir(uploadFolder, async (err, files) => {
                ...
                await Category.update({ image: newImageName }, { where: { id: categoryId } });
            })
...
const deleteCategoryById = async (req, res) => {
    ...
    try {
        ...
        if (deletedCategory) {
            res.status(200).json({ message: 'category and related products deleted successsfully', success: true, });
        } else {
